...
public interface IMedicalLetterService
{
    ...
    Task<IEnumerable<MedicalLetterSummaryResponseDTO>> GetPreviousLettersAsync(string doctorId, string patientId, Guid? excludeSessionId, CancellationToken ct);
    Task<MedicalLetterResponseDTO> GetByIdAsync(Guid id);
    Task<MedicalLetterResponseDTO> GetBySessionIdAsync(Guid sessionId);
    Task<MedicalLetterResponseDTO> UpdateLetterAsync(Guid id, MedicalLetterPutDTO dto, string doctorId, CancellationToken ct);
